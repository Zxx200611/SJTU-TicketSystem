...
#pragma pack(1)
// Node_old ***********************************************************
template <typename T, typename Comp,int N,int M> Node_old<T, Comp,N,M>::Node_old() noexcept {
  pos = 0, ch_cnt = 0;
...
Node_old<T, Comp,N,M>::Node_old(int this_pos, FileOperator &fo,
...
template <typename T, typename Comp,int N,int M>
Node_old<T,Comp,N,M> &Node_old<T,Comp,N,M>::operator=(const Node_old<T,Comp,N,M> &b) {
  if (&b == this)
    return *this;

  memcpy(this, &b, sizeof(Node_old));
...
template <typename T, typename Comp,int N,int M>
void Node_old<T,Comp,N,M>::write(FileOperator &fo,
                             MemoryRiver_old<T,Comp,N,M> &mr) noexcept {
  mr.writeNode(*this, fo);
}
template <typename T, typename Comp,int N,int M>
constexpr T Node_old<T,Comp,N,M>::maxElement() noexcept {
  return ch_dat[ch_cnt - 1];
}
template <typename T, typename Comp,int N,int M>
constexpr bool Node_old<T,Comp,N,M>::isLeaf() noexcept {
  return ch_pos[0] == 0;
}
template <typename T, typename Comp,int N,int M>
void Node_old<T,Comp,N,M>::insert(int p, const T &v, int pos) noexcept {
    if (p < ch_cnt) {
...
template <typename T, typename Comp,int N,int M>
void Node_old<T,Comp,N,M>::remove(int p) noexcept {
    if (p + 1 < ch_cnt) {
...
template <typename T, typename Comp,int N,int M>
int Node_old<T,Comp,N,M>::findPos(int p) noexcept {
  for (int i = 0; i < ch_cnt; i++)
    if (ch_pos[i] == p)
      return i;
  assert(0);
...
// MR *************************************************************
template <typename T, typename Comp,int N,int M>
MemoryRiver_old<T,Comp,N,M>::MemoryRiver_old() noexcept {
  // do nothing
}
template <typename T, typename Comp,int N,int M>
void MemoryRiver_old<T,Comp,N,M>::clear() noexcept {
  std::memset(mem,0,sizeof(mem));
}
template <typename T, typename Comp,int N,int M>
void MemoryRiver_old<T,Comp,N,M>::readNode(int pos, FileOperator &fo,
...
template <typename T, typename Comp,int N,int M>
void MemoryRiver_old<T,Comp,N,M>::writeNode(Node_old<T,Comp,N,M> u,
...
template <typename T, typename Comp,int N,int M>
void MemoryRiver_old<T,Comp,N,M>::invalidate(int pos) noexcept {
...
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::updateAncestors(int uid, int vp) noexcept {
  Node_old<T,Comp,N,M> v(vp, fo, mr);

  while (uid != 0) {
    Node_old<T,Comp,N,M> u(uid, fo, mr);
...
template <typename T, typename Comp,int N,int M>
std::pair<int, int> BPlusTree_old<T,Comp,N,M>::innerInsert(int uid,
                                                       const T &t) noexcept {
  Node_old<T,Comp,N,M> u(uid, fo, mr);
  int ips =
  ...
  if (u.isLeaf())
    ...
    if (pr.first != 0) {
      ...
      u.insert(ips, Node_old<T,Comp,N,M>(pr.first, fo, mr).maxElement(), pr.first);
    }
  }

  if (u.ch_cnt == Node_old<T,Comp,N,M>::max_ch_cnt + 1) {
    Node_old<T,Comp,N,M> &L = u, R;
    R.pos=fo.allocate();
    if (L.isLeaf())
    ...
    if (!R.isLeaf()) {
      for (int i = 0; i < R.ch_cnt; i++) {
        Node_old<T,Comp,N,M> tmp(R.ch_pos[i], fo, mr);
        tmp.fth = R.pos;
        tmp.write(fo, mr);
      }
    }
...
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::innerRemove(int uid, int vid, const T &t,bool ulev) noexcept {
  Node_old<T,Comp,N,M> u(uid, fo, mr);
  int ips =
      utils::lowerBound(u.ch_dat, u.ch_dat + u.ch_cnt, t, comp) - u.ch_dat;
  assert(ips != u.ch_cnt);
  ...
  if (u.isLeaf()) {
    // std::cout<<"Find deleting element at Node_old #"<<uid<<" 's "<<ips<<"th
    // pos."<<std::endl;
    u.remove(ips);
    u.write(fo, mr);
    if (ips == u.ch_cnt && u.ch_cnt > 0)
      updateAncestors(u.fth, u.pos);
    if (u.ch_cnt >= (Node_old<T,Comp,N,M>::max_ch_cnt) / 2)
      return;
  } else {
    ...
    u = Node_old<T,Comp,N,M>(uid, fo, mr);
    if (u.ch_cnt >= (Node_old<T,Comp,N,M>::max_ch_cnt) / 2)
      return;
  }
  ...
  if (u.pos == rt_pos) {
    if (u.ch_cnt == 1 && u.ch_pos[0] != 0) {
      Node_old<T,Comp,N,M> tmp(u.ch_pos[0], fo, mr);
      tmp.fth = 0;
      tmp.write(fo, mr);
  ...
  Node_old<T,Comp,N,M> v(vid, fo, mr);
  if (v.ch_cnt + u.ch_cnt <= Node_old<T,Comp,N,M>::max_ch_cnt) {
    if (!ulev) // make sure u<v
    {
      std::swap(u, v);
    }
    // std::cout<<"Merge it with Node_old #"<<vid<<std::endl;
    ...
    for (int i = 0; i < v.ch_cnt; i++) {
      ...
      if (!u.isLeaf()) {
        Node_old<T,Comp,N,M> tmp(v.ch_pos[i], fo, mr);
        tmp.fth = u.pos;
        tmp.write(fo, mr);
      }
    }
    ...
    if (u.fth != 0) {
      Node_old<T,Comp,N,M> f(u.fth, fo, mr);
    ...
    if (ulev) {
      ...
      if (v.ch_pos[0] != 0) {
        Node_old<T,Comp,N,M> tmp(v.ch_pos[0], fo, mr);
        tmp.fth = u.pos;
        tmp.write(fo, mr);
      }
      ...
      if (v.ch_pos[v.ch_cnt - 1] != 0) {
        Node_old<T,Comp,N,M> tmp(v.ch_pos[v.ch_cnt - 1], fo, mr);
        tmp.fth = u.pos;
        tmp.write(fo, mr);
      }
...
template <typename T, typename Comp,int N,int M>
std::pair<int, int> BPlusTree_old<T,Comp,N,M>::lowerBound(const T &t) noexcept {
  Node_old<T,Comp,N,M> u(rt_pos, fo, mr);
  while (!u.isLeaf()) {
    int v_id =
        utils::lowerBound(u.ch_dat, u.ch_dat + u.ch_cnt, t, comp) - u.ch_dat;
    assert(v_id < u.ch_cnt);
    u = Node_old<T,Comp,N,M>(u.ch_pos[v_id], fo, mr);
  }
...
template <typename T, typename Comp,int N,int M>
BPlusTree_old<T,Comp,N,M>::BPlusTree_old(const std::string &name) noexcept: comp(), fo(name+".dat"), siz(0), mr() {
  if (fo.size() != 0) {
    fo.read(0, &rt_pos);
  ...
  Node_old<T,Comp,N,M> u(sizeof(int) * 2, fo, mr);
  // std::cout<<"Node_old constructed"<<std::endl;
  ...
  // u=Node_old<T,Comp>(sizeof(int)*2,fo,mr);
  // debugPrint(std::cerr);
  // std::cerr<<"root's ch_cnt = "<<u.ch_cnt<<std::endl;
}
template <typename T, typename Comp,int N,int M>
BPlusTree_old<T,Comp,N,M>::~BPlusTree_old() noexcept {
  fo.write(0, &rt_pos);
  fo.write(sizeof(int), &siz);

  for (int i = 0; i < MemoryRiver_old<T,Comp,N,M>::mr_siz; i++) {
    if (mr.mem[i].pos != 0)
      fo.write(mr.mem[i].pos, &mr.mem[i]);
  }
}
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::clear() noexcept {
...
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::insert(const T &t) noexcept {
  siz++;
  std::pair<int, int> pr = innerInsert(rt_pos, t);
  if (pr.first == 0)
    return;

  Node_old<T,Comp,N,M> u(pr.first, fo, mr), v(pr.second, fo, mr),w;
  w.pos=fo.allocate();
...
template <typename T, typename Comp,int N,int M>
bool BPlusTree_old<T,Comp,N,M>::remove(const T &t) noexcept {
  std::pair<int, int> pr = lowerBound(t);
  if (comp(t, Node_old<T,Comp,N,M>(pr.first, fo, mr).ch_dat[pr.second]))
    return 0;
...
template <typename T, typename Comp,int N,int M>
sjtu::vector<T> BPlusTree_old<T,Comp,N,M>::find(const T &l, const T &r) noexcept {
...
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::innerDebugPrint(int uid,std::ostream &os) noexcept {
  Node_old<T,Comp,N,M> u(uid, fo, mr);
  os << "Node_old #" << uid << " : pos = " << u.pos
...
template <typename T, typename Comp,int N,int M>
void BPlusTree_old<T,Comp,N,M>::debugPrint(std::ostream &os) noexcept {
  os << "BPT ***************************" << std::endl;
