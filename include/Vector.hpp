...
public:
  ...
  public:
    ...
    pointer position() const { return pos; }
    const_iterator(const vector<T> *_bel, pointer _pos) noexcept
        : bel(_bel), pos(_pos) {}
    const_iterator operator+(const int &n) const {
      return const_iterator(bel, pos + n);
    }
    const_iterator operator-(const int &n) const {
      return const_iterator(bel, pos - n);
    }
    ...
    const_iterator operator++(int) {
      pos++;
      return const_iterator(bel, pos - 1);
    }
...
template <typename T> const int vector<T>::initial_size = 4;
template <typename T> const int vector<T>::scaleup_rate = 2;
